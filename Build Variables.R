...
ggplot(for_graph[1:15,],aes(x=PCnum,y=var_expl))+
  ...
  ggtitle("Amount of Variance Explained by each PC")+
...
remainder=nrow(NY_auto_scorer)%%bin_number
interval=nrow(score2_data)-remainder
iter=interval/bin_number

NY_auto_scorer$bin_num =0
NY_auto_scorer%>%
  arrange(-scoreEuc) -> final_auto_binned
for(i in c(1:bin_number)){
  beg = (i-1)*iter+1
  end=i*iter
  final_auto_binned$bin_num[beg:end]=bin_number+1-i
}
...
final_auto_binned%>%
  ...
  nrow()
final_auto_binned%>%
  mutate(bin_num=if_else(bin_num==0,1,bin_num)) ->final_auto_binned

final_auto_binned%>%
  filter(scoreEuc<5)%>%
  ggplot(aes(x=scoreEuc))+
  geom_histogram(bins=100)

##write out this dataset the top 4000 records and all the records
write.csv(final_auto_binned,"final_auto_binned_euclidean.csv")
final_auto_binned%>%
  head(4000) %>%
  write.csv("final_auto_binned_euclidean_top4000.csv")

###combine the larger dataset to this
all_records_binned_autoencoder = left_join(final_auto_binned,prop_data)

##write out all records and the top 4000 records
write.csv(all_records_binned_autoencoder,"NY_allrecords_autoencoder_binned.csv")

all_records_binned_autoencoder%>%
  head(4000)%>%
  write.csv("NY_top4000_autoencoder_binned.csv")


##take average of the bin numbers to find final score
all_records_binned_autoencoder%>%
  arrange(RECORD)-> all_records_binned_autoencoder

score2_records%>%
  head()

all_records_binned_autoencoder$bin_num_heuristic=score2_records$bin_num
all_records_binned_autoencoder$scoreEuc_algo1=score2_records$score2
##summ the bin numbers 
wt1 = 0.5
wt2 = 1-wt1

##define the combined score
all_records_binned_autoencoder$combined_score=all_records_binned_autoencoder$bin_num*wt2+wt1*all_records_binned_autoencoder$bin_num_heuristic
all_records_binned_autoencoder%>%
  select(-scoreMan,-bin_num,-bin_num_heuristic) -> all_records_binned_autoencoder
###sort the records and write out the top 4000 and all records
all_records_binned_autoencoder%>%
  arrange(-combined_score)%>%
  write.csv("Final_Records_Combined_Scores.csv")

all_records_binned_autoencoder%>%
  arrange(-combined_score) %>%
  head(400)%>%
  write.csv("Final_400Records_Combined_Scores.csv")

ggplot(all_records_binned_autoencoder,aes(x=combined_score))+
